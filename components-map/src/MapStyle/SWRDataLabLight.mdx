...
import Mermaid from '../../../storybook/.storybook/blocks/Mermaid';
...
<table>
	...
	<tr>
		<td>Hillshading</td>
		<td>[versatiles-elevation](https://download.versatiles.org/) vector tiles based on [Mapterthorn terrain data](https://mapterhorn.com/) served from `tiles.datenhub.net`</td>
    <td>[Copernicus Regulation EU No 377/2014](https://docs.versatiles.org/basics/tilesets.html#elevation)</td>
    </tr>
    <tr>
...
export const customIconsCount = Object.entries(SpriteData).filter((row) => {
	return customIcons.includes(row[0]);
}).length;

SWRDataLabLight includes {osmicIconsCount} icons from [Osmic](https://github.com/gmgeo/osmic), a public-domain set of map icons, and {customIconsCount} custom icon{customIconsCount > 1 ? "s": ""} ({Object.keys(SpriteData).length} overall). Custom icons live in [this Figma file](https://www.figma.com/design/2MbVC3SYyyahO2UPuy3z0S/Map-Icons?node-id=0-1&t=IJuwwIj86FGIjRGa-1). See `VectorLayer` for usage.
