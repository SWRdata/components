...
export type MaplibreGeocoderFeatureResults = {
...
export type MaplibreGeocoderResults =
	| MaplibreGeocoderFeatureResults
	| MaplibreGeocoderSuggestionResults
	| MaplibreGeocoderPlaceResults;

export type MaplibreGeocoderResultsEvent = MaplibreGeocoderResults & {
	config?: MaplibreGeocoderApiConfig;
	features?: CarmenGeojsonFeature[];
};

export type TypeaheadFactory = (
	input: HTMLInputElement,
	data: (CarmenGeojsonFeature | MaplibreGeocoderSuggestion)[],
	options: TypeaheadOptions
) => Typeahead<CarmenGeojsonFeature | MaplibreGeocoderSuggestion>;
// Re-export
...
export default class MaplibreGeocoder {
	...
	constructor(
		...
		typeaheadFactory: TypeaheadFactory = (input, data, options) =>
			new Typeahead(input, data, options)
	) {
	...
	_onChange() {
		...
		if (this.options.flyTo) {
			...
			if (selected.properties && exceptions[selected.properties.short_code]) {
				...
				} else if (selected.geometry?.type === 'Point' && selected.geometry.coordinates) {
	...
	_getConfigForRequest(): MaplibreGeocoderApiConfig {
		...
		const keys = ['bbox', 'limit', 'proximity', 'countries', 'types', 'language', 'reverseMode'];
		...
		const config: MaplibreGeocoderApiConfig = keys.reduce((config, key) => {
			if (this.options[key]) {
				...
				if (
					...
					config[key] = [this.options[key].longitude, this.options[key].latitude];
				}
			}
			return config;
		}, {});
	...
	async _geocode(
		...
		isSuggestion = false,
		isPlaceId = false
	): Promise<MaplibreGeocoderResults> {
	...
	private _localize(key: keyof typeof localization): string {
		...
		return this.options.language && localization?.[key][language]
	...
	setPlaceholder(placeholder?: string): this {
		this.options.placeholder = placeholder
			? placeholder
			: this.options.placeholder || this._localize('placeholder');
	...
	private _handleResultMarkers(results: any[]): this {
		...
		let markerOptions = extend({}, defaultMarkerOptions, this.options.showResultMarkers as any);
	 ...
	 */
	on<T extends keyof MaplibreGeocoderEventTypeMap>(
		type: T,
		fn: (e: MaplibreGeocoderEventTypeMap[T]) => void
	): this {
	 ...
	 */
	once<T extends keyof MaplibreGeocoderEventTypeMap>(
		type: T
	): Promise<MaplibreGeocoderEventTypeMap[T]> {
